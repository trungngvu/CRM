...
const TaskDetail = (): JSX.Element => {
  ...
  const handleDeleteTask = async () => {
    await deleteTask({ id });
    close();
    navigate(`${PAGES.JOB_LIST}?projectId=${projectId}`);
  ...
  const handleClickBack = () => {
    if (location.state?.from === PAGES.PLAN_DETAIL) return navigate(`${PAGES.PLAN_DETAIL}?id=${projectId}`);
    if (location.state?.from === PAGES.JOB_DETAIL)
      return navigate(`${PAGES.JOB_DETAIL}?id=${location.state?.parentId}&projectId=${projectId}`);
    return navigate(`${PAGES.JOB_LIST}?projectId=${projectId}`);
  };

  const handleEdit = () => navigate(`${PAGES.UPDATE_JOB}?projectId=${projectId}&taskId=${id}`);
  ...
  const childTasks = (taskDetail?.children || []).map((item, index) => ({
    ...
    task_id: (
      <TextLink
        state={{ from: PAGES.JOB_DETAIL, parentId: `${id}` }}
        to={`${PAGES.JOB_DETAIL}?id=${item.id}&projectId=${projectId}`}
      >
        {item?.id}
      </TextLink>
    ),
