...
const TaskList = (): JSX.Element => {
  ...
  const goToAddTask = () => history.push(`${PAGES.ADD_JOB}?projectId=${projectId}`);
  ...
  const tableData = filteredData.map(
    ({
      ...
      id,
      TASK_CODE: <TextLink to={`${PAGES.JOB_DETAIL}?id=${id}&projectId=${projectId}`}>{id}</TextLink>,
      TASK_NAME: (
        <TextLink to={`${PAGES.JOB_DETAIL}?id=${id}&projectId=${projectId}`} className="w-full text-left">
          {name}
        </TextLink>
      ),
